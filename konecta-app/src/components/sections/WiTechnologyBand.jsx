...
import { witechnologyStats } from "@data/events";
...
function WiStat({ value, label }) {
  ...
  return (
    <div ref={ref} className="glass-card p-6 text-center">
      <div
        className="font-heading font-extrabold text-gradient-orange"
...
export default function WiTechnologyBand() {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 text-center max-w-4xl mx-auto">
        ...
        >
          <div className="section-eyebrow text-konecta-orange justify-center">
            WiTechnology Initiative
          </div>
          <h2
            className="font-heading font-extrabold text-konecta-white leading-[1.05] mt-4"
          ...
          <p className="text-base text-white/75 leading-relaxed mt-6 max-w-2xl mx-auto">
            The WiTechnology Summit brings together Africa&apos;s top telecoms
            operators, technology leaders, policymakers, and innovators to shape
            the continent&apos;s digital future.
          </p>
        </div>

        <div
          className={`grid grid-cols-2 md:grid-cols-4 gap-4 mt-16 transition-all duration-700 delay-200 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
          {witechnologyStats.map((s) => (
            <WiStat key={s.label} value={s.value} label={s.label} />
          ))}
        </div>

        <div
          className={`flex gap-4 justify-center flex-wrap mt-12 transition-all duration-700 delay-400 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
          <Link
            to="https://witechnology.africa/"
            target="_blank"
